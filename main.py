...
def datewise_summary_table_v2(show=None, hafiz_id=None):
    qry = f"SELECT MIN(revision_date) AS earliest_date FROM {revisions}"
    qry = (qry + f" WHERE hafiz_id = {hafiz_id}") if hafiz_id else qry
    result = db.q(qry)
    earliest_date = result[0]["earliest_date"]
    ...
    return Title("Quran SRS"), Container(
        Div(
            NavBar(
                ...
                A("Report v3", href="/report_v3", cls=is_active("Report v3")),
                # A(
                #     "New Memorization",
...
def datewise_summary_table_view_v2(auth):
    return main_area(
        datewise_summary_table_v2(hafiz_id=auth), active="Report v2", auth=auth
    )
...
def report_v3(auth):
    return main_area(
        Div(datewise_summary_table(hafiz_id=auth), cls="block md:hidden"),
        Div(datewise_summary_table_v2(hafiz_id=auth), cls="hidden md:block"),
        active="Report v3",
        auth=auth,
    )
